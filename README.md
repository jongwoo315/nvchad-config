...
| `<C-d>` / `<C-f>` / `PageDown` | 반 페이지 / 한 페이지 아래 (EOF 아래 빈 공간 스크롤 방지 커스텀) |

- 가로 스크롤은 nowrap 창에서만 의미 있음. dap-ui/diffview 패널은 자체적으로 nowrap 설정 — 긴 줄이 잘리면 그 창에서 `:setlocal wrap`으로 줄바꿈 전환 가능.
- `<A-…>` = macOS에선 Option(⌥) 키. `⌥+,`가 `≤` 같은 특수문자로 입력되면 터미널의 "Option as Alt/Meta" 설정 활성화 필요.
...
| `gl` | diagnostic 전체 메시지 플로팅 (virtual text 잘릴 때 — 한 번 더 누르면 창 진입, yank 가능) |
